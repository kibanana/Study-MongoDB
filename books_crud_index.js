// 저자로 조회하는 일이 많으므로 저자 필드에 인덱스를 설정한다.
...
db.getCollection('books').update({presents: "bookmark"}, {$pop: {'bookmark'}})// $pop : 맨 앞 또는 맨 뒤 요소를 꺼낸다. -1 -> shift, 1 -> pop
db.getCollection('books').update({}, {$pull: {author: "Kim Jin Seong"}}) // $pull : 배열에서 조건을 만족하는 특정한 요소를 꺼낸다.

// $pullAll : 조건으로 넘긴 배열에서 일치하는 값을 배열에서 꺼낸다.
db.getCollection('books').update({}, {$pullAll: {author: ["Jo Ho Muk", "Kang Ik Sun"]}})

// $push : 배열에 요소를 push한다
db.getCollection('books').update({}, {$push: {review: { $each: [{}, {}, {}] }}})

db.getCollection('books').update({}, {}) // $each -> push, addToSet과 같이 사용하는 경우가 있다 -> 인자로 배열을 전달한다
